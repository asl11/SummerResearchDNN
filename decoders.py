...
class DenseNNRegression_Hanlin(object):
    ...
    ADDITIONAL ADDED PARAMS:
    LA2: 10^LA2 is used as the l2 regularization
    lr: Learning Rate is manually inputted into rms_prop
    """

    def __init__(self,units=400,dropout=0,num_epochs=10,verbose=2,LA2=0.0, lr=0.004,savePath =''):
    ...
    def fit(self,X_train,y_train,X_valid, y_valid):
        ...
        from keras.layers import DepthwiseConv2D, Flatten, SeparableConv1D, Conv2D
        ...
        Parameters
        ----------
        X_flat_train: numpy 2d array of shape [n_samples,n_features], [n_samples,n_time_bins,n_neurons]
            This is the neural data.
            See example file for an example of how to format the neural data correctly
        ...
        # Add temporal convolution layer, assume using 16 bin, apply 8 different temporal filters
        #model.add(Conv2D(12,(25,1), strides=(1, 1),data_format="channels_last",kernel_constraint=NonNeg()))
        #model.add(Conv2D(12, (25, 1), strides=(1, 1), data_format="channels_last"))
        model.add(Flatten(input_shape=(25,2548,1)))
        # Add first hidden layer
        model.add(Dense(self.units[0],kernel_regularizer=regularizers.l2(self.LA2),activation="swish"))#Add dense layer
        # model.add(Activation(swish,name='SpecialActivation')) #Add nonlinear (tanh) activation
        # if self.dropout!=0:
        ...
        for layer in range(self.num_layers-1): #Loop through additional layers
            model.add(Dense(self.units[layer+1],kernel_regularizer=regularizers.l2(self.LA2),activation="swish")) #Add dense layer
            # model.add(Activation(swish,name='SpecialActivation')) #Add nonlinear (tanh) activation
            if self.dropout!=0: model.add(Dropout(self.dropout)) #Dropout some units if proportion of dropout != 0
        ...
        def R2(y_true, y_pred):
            ...
            return 1 - K.sum((y_pred - y_true) ** 2) / (K.sum((y_true - y_mean) ** 2)+0.000001)
        #Fit model (and set fitting parameters)
        model.compile(loss='mse',optimizer=rmsprop(learning_rate=self.lr),metrics=[R2]) #Set loss function and optimizer, previous lr was set 0.004
        model.fit(X_train,y_train,epochs=self.num_epochs,verbose=self.verbose, batch_size=128,validation_data=(X_valid, y_valid)) #Fit the model
        model.summary()
        plot_model(model, to_file=self.savePath + 'model.png', expand_nested=True, show_shapes=True, dpi=300)
        print(self.LA2)
        print("learning rate: " + str(self.lr))
        print(self.dropout)
        model.save(self.savePath + 'my_model.h5')
        self.model=model
...
class DenseNNRegression_Hanlin_IntSuperAdd(object):
    ...
    def __init__(self,units=400,dropout=0,num_epochs=10,verbose=2,LA2=0.0,savePath =''):
         self.dropout=dropout
         self.num_epochs=num_epochs
         self.verbose=verbose
         self.LA2=LA2
    ...
    def fit(self,X_train,y_train,X_valid, y_valid):
        from keras.constraints import NonNeg
        from keras.layers import DepthwiseConv2D, Flatten, SeparableConv1D, Conv2D, Add
        ...
        lst = Input(shape=(X_train.shape[1], X_train.shape[2],X_train.shape[3]),name='lst')
        x1 = Conv2D(12, (25, 1), strides=(1, 1), data_format="channels_last", kernel_constraint=NonNeg())(lst)
        x1 = Flatten()(x1)
        # Add first hidden layer
        x1 = Dense(self.units[0], kernel_regularizer=regularizers.l1(self.LA2))(x1)
        # Add dense layer
        x1 = Activation('relu')(x1)  # Add nonlinear (tanh) activation
        # if self.dropout!=0:
        if self.dropout != 0: x1 = Dropout(self.dropout)(x1)  # Dropout some units if proportion of dropout != 0
        # Add any additional hidden layers (beyond the 1st)
        for layer in range(self.num_layers - 1):  # Loop through additional layers
            x1= Dense(self.units[layer + 1])(x1)  # Add dense layer
            x1= Activation('relu')(x1) # Add nonlinear (tanh) activation
        if self.dropout != 0: x1 = Dropout(self.dropout)(x1)  # Dropout some units if proportion of dropout != 0
        # Add dense connections to all outputs
        y1 = Dense(12,name='y1')(x1)

        x2 = Conv2D(12, (25, 1), strides=(1, 1), data_format="channels_last", kernel_constraint=NonNeg())(lst)
        x2 = Flatten()(x2)
        # Add first hidden layer
        x2 = Dense(self.units[0], kernel_regularizer=regularizers.l1(self.LA2))(x2)
        # Add dense layer
        x2 = Activation('relu')(x2)  # Add nonlinear (tanh) activation
        # if self.dropout!=0:
        if self.dropout != 0: x2 = Dropout(self.dropout)(x2)  # Dropout some units if proportion of dropout != 0
        # Add any additional hidden layers (beyond the 1st)
        for layer in range(self.num_layers - 1):  # Loop through additional layers
            x2 = Dense(self.units[layer + 1])(x2)  # Add dense layer
            x2 = Activation('relu')(x2)  # Add nonlinear (tanh) activation
        if self.dropout != 0: x2 = Dropout(self.dropout)(x2)  # Dropout some units if proportion of dropout != 0
        # Add dense connections to all outputs
        y2 = Dense(12,activation='relu',name='y2')(x2)


        x3 = concatenate([x1, x2])
        y3 = Dense(12,name='y3')(x3)
        model = Model(inputs=[lst], outputs=[y1,y2,y3])
        model.compile(optimizer=adam(0.004,0.8),
                      loss={'y1': 'mse', 'y2': 'mse','y3': 'mse'},
                      loss_weights={'y1': 1., 'y2': 1., 'y3':0.5})
        model.summary()
        print(self.units)
        print(self.LA2)
        print(self.dropout)
        plot_model(model, to_file=self.savePath+'model.png', expand_nested=True, show_shapes=True, dpi=300)
        model.fit({'lst': X_train},{'y1': y_train[:,0:12], 'y2': y_train[:,12:24],'y3': y_train[:,24:36]},
                  epochs=self.num_epochs, verbose=self.verbose, batch_size=128,validation_data=({'lst': X_valid},
                  {'y1': y_valid[:,0:12], 'y2': y_valid[:,12:24],'y3': y_valid[:,24:36]}))
        model.save(self.savePath + 'my_modelISA.h5')
        self.model = model
...
class DenseNNRegression(object):
    ...
    def fit(self,X_flat_train,y_train):
        ...
        Parameters
        ----------
        X_train: numpy 3d array of shape [n_samples,n_features] [n_samples,n_time_bins,n_neurons]
            This is the neural data.
            See example file for an example of how to format the neural data correctly
        ...
        model=Sequential() #Declare model
        model.add(SimpleRNN(self.units, input_shape=(X_train.shape[1], X_train.shape[2]), dropout_W=self.dropout,
                            dropout_U=self.dropout, activation='relu'))
        #Add first hidden layer
...
class SimpleRNNRegression(object):
    ...
    def fit(self,X_train,y_train):
        ...
        model.fit(X_train,y_train,nb_epoch=self.num_epochs,verbose=self.verbose,batch_size=10) #Fit the model
        self.model=model
...
class LSTMRegression(object):
    ...
    def __init__(self,units,dropout,num_epochs,batch_size,lr,final_act,verbose=2):
         self.units=units
         self.dropout=dropout
         self.num_epochs=num_epochs
         self.verbose=verbose
    ...
    def fit(self,X_train,y_train,X_valid,y_valid):
        ...
        es = EarlyStopping(monitor='val_loss', verbose=1, patience=initialEpoch, min_delta=0.1, baseline=baseline)
        model=Sequential() #Declare model
        #Add recurrent layer
        # model.add(Dropout(0.9, input_shape=(X_train.shape[1],X_train.shape[2])))
        model.add(LSTM(self.units,input_shape=(X_train.shape[1],X_train.shape[2]),dropout=self.dropout,recurrent_dropout=self.dropout,unroll=True,return_sequences=False,kernel_regularizer=regularizers.l2(0.0))) #Within recurrent layer, include dropout
        # model.add(Bidirectional(LSTM(5, dropout=self.dropout,
        #                              recurrent_dropout=self.dropout, unroll=True,return_sequences=True)))
        # model.add(Bidirectional(LSTM(3, dropout=self.dropout,
        #                              recurrent_dropout=self.dropout, unroll=True)))
        #if self.dropout!=0: model.add(Dropout(self.dropout)) #Dropout some units (recurrent layer output units)
        optimizer = Adadelta(lr = self.lr)
        #model.add(Dropout(self.dropout))
        #Add dense connections to output layer
        model.add(Dense(y_train.shape[1],activation=self.final_act,kernel_regularizer=regularizers.l2(0.0)))
        #Fit model (and set fitting parameters)

        model.compile(loss='mse',optimizer=optimizer,metrics=[R2]) #Set loss function and optimizer
        print('frac_dropout: ', self.dropout)
        print('num_units: ', self.units)
        his = model.fit(X_train, y_train, epochs=initialEpoch, verbose=self.verbose, batch_size=self.batch_size,
                  validation_data=(X_valid, y_valid),callbacks=[es])
        model.summary()
        loss=model.evaluate(X_valid, y_valid)
        print(loss[0])
        if loss[0]<baseline:
            es = EarlyStopping(monitor='val_loss', verbose=1, patience=16, min_delta=0.01,restore_best_weights=True)
            model.fit(X_train, y_train, epochs=self.num_epochs, initial_epoch=initialEpoch,verbose=self.verbose, batch_size=self.batch_size,
                      validation_data=(X_valid, y_valid), callbacks=[es])  # Fit the model
...
class LSTMClassification(object):
    ...
    def __init__(self,units,dropout,num_epochs,batch_size,lr,final_act,LD2=0.0,LA2=0.0,verbose=1):
         self.units=units
         self.dropout=dropout
         self.num_epochs=num_epochs
         self.verbose=verbose
    ...
    def fit(self,X_train,y_train,X_valid, y_valid,NRUPL,NDPL,NAPL):
        """
        Train LSTM Decoder
        Parameters
        ----------
        ...
        if y_train.ndim==1:
            ...
            y_train=np_utils.to_categorical(y_train.astype(int))
        seqLen =  X_train[0].shape[1]
        lst = [Input(shape=(seqLen,NRUPL[i])) for i in range(18)]
        listD = []
        listM = []
        for i in range(18):
            # lst[i].add(Dropout(self.dropout))
            listD.append(Dense(int(NDPL[i]),activation=self.final_act, kernel_regularizer=regularizers.l2(self.LD2))(lst[i]))
            listM.append(LSTM(int(NAPL[i]), dropout=self.dropout,
                       recurrent_dropout=self.dropout, unroll=True,
                       kernel_regularizer=regularizers.l2(self.LA2),return_state=False,return_sequences=False)(listD[i]))

        x = concatenate(listM)
        x = BatchNormalization()(x)
        x = Dropout(self.dropout)(x)
        x = Dense(y_train.shape[1], activation='sigmoid', kernel_regularizer=regularizers.l2(0.0))(x)


        model = Model(inputs=lst, outputs=x)
        model.summary()
        plot_model(model, to_file='model.png',expand_nested=True,show_shapes=True,dpi=300)
        optimizer = Adam(lr=self.lr)
        model.compile(loss='categorical_crossentropy',optimizer=optimizer,metrics=['accuracy']) #Set loss function and optimizer
        model.fit(x=X_train,y=y_train,epochs=self.num_epochs,verbose=self.verbose,validation_data=(X_valid, y_valid),batch_size=512) #Fit the model
        self.model=model
...
class LSTMClassification_sim(object):
    ...
    def __init__(self,units,dropout,num_epochs,batch_size,lr,final_act,LD2=0.0,LA2=0.0,verbose=2):
         self.units=units
         self.dropout=dropout
         self.num_epochs=num_epochs
         self.verbose=verbose
    ...
    def fit(self,X_train,y_train,X_valid, y_valid,NRUPL,NDPL,NAPL):
        ...
        lst = Input(shape=(seqLen,X_train.shape[2]) )
        x = LSTM(self.units,
             recurrent_dropout=self.dropout, unroll=True,
             kernel_regularizer=regularizers.l1(self.LA2), return_state=False, return_sequences=False)(lst)
        x = Dropout(self.dropout)(x)
        x = Dense(y_train.shape[1], activation='softplus', kernel_regularizer=regularizers.l2(0.0))(x)
        ...
        model.compile(loss='categorical_crossentropy',optimizer=optimizer,metrics=['accuracy']) #Set loss function and optimizer
        model.fit(x=X_train,y=y_train,epochs=self.num_epochs,verbose=self.verbose,validation_data=(X_valid, y_valid),batch_size=128) #Fit the model
        self.model=model
